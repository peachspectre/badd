...
// LINE Channel Credentials - ใส่ค่าจริงจาก LINE Developers Console
...
const GROUP_ID = 'Cc4b0c3a420fc92265e01e1236d666969'; // ไม่จำเป็นต้องใช้สำหรับ Bot ทั่วไป
...
async function handleEvent(event) {
  ...
  // ตรวจสอบว่าผู้ใช้เคยได้รับข้อความต้อนรับแล้วหรือยัง
  const userId = event.source.userId;
  if (!userStates[userId] || !userStates[userId].welcomed) {
    // ส่งข้อความต้อนรับครั้งแรกที่ผู้ใช้ส่งข้อความมา
    userStates[userId] = { welcomed: true };
    await client.replyMessage(event.replyToken, {
      type: 'text',
      text: `สวัสดีค่ะ น้องแบดตี้ยินดีให้บริการท่านเทพแบดมินตันกฤษในตำนาน 🏸✨\n\n` +
            `ถ้าต้องการให้น้องแบดตี้ช่วยสรุปค่าใช้จ่ายในการตีแบดให้พิมพ์คำว่า "น้องแบดตี้สรุปค่าตีแบดให้หน่อยจ้า" ได้เลยนะคะ 😊`,
    });
    return;
  }
  ...
  console.log('📝 ข้อความที่ได้รับ:', userMessage);
  console.log('📝 ความยาวข้อความ:', userMessage.length);
  console.log('📝 Character codes:', userMessage.split('').map(c => c.charCodeAt(0)));
  
  if (userMessage.includes('น้องแบดตี้สรุปค่าตีแบดให้หน่อยจ้า')) {
    console.log('🎯 ตรงกับคำสั่งเริ่มต้น!');
    userStates[event.source.userId] = { step: 'waiting_date', data: {}, welcomed: true };
  ...
  // ถ้าไม่ใช่คำสั่งเริ่มต้นและไม่อยู่ในขั้นตอนการคำนวณ
  return client.replyMessage(event.replyToken, {
    type: 'text',
...
app.get('/', (req, res) => {
  res.send(`
    <h1>🏸 น้องแบดตี้ LINE Bot</h1>
    <p>เซิร์ฟเวอร์ทำงานปกติแล้วครับ!</p>
    <p>สามารถใช้งานผ่าน LINE Bot ได้เลย</p>
